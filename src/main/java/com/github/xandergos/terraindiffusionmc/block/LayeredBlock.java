...
public class LayeredBlock extends Block implements Waterloggable {
    ...
    /** True if the block is waterlogged. Layer 1 is too thin to hold water and stays {@code false}. */
    ...
    public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        if (existing.isOf(this)) {
            ...
            if (current < 7) {
                return existing.with(LAYERS, current + 1);
            }
            // 7 -> 8: promote to the vanilla full cube, preserving waterlogging when supported.
    ...
    public FluidState getFluidState(BlockState state) {
        return state.get(LAYERS) >= 2 && state.get(WATERLOGGED)
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView tickView,
        ...
        if (state.get(LAYERS) >= 2 && state.get(WATERLOGGED)) {
